...
class ProjectFormController extends Controller
{
    ...
    public function store(BeneficiaryStoreRequest $request, Project $project){
        ...
        $beneficiary = Benefitiary::create([
            ...
            'internal_status' => auth()->user()->can('approve-beneficiaries',[$project,$this->roles]) ? Benefitiary::INTERNAL_STATUSES['approved'] : Benefitiary::INTERNAL_STATUSES['pending'],
            'approved_at' => auth()->user()->can('approve-beneficiaries',[$project,$this->roles]) ? now() : null,
        ]);
