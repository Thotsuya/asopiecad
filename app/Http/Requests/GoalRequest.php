...
class GoalRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'conditions.*.conditions.*.form_id' => ['required', 'numeric', 'min:1', 'exists:forms,id'],
            'conditions.*.conditions.*.form_slug' => ['required', 'string', 'max:255'],
            'conditions.*.conditions.*.field_id' => ['required', 'numeric', 'min:1'],
            'conditions.*.conditions.*.field_slug' => ['required', 'string', 'max:255'],
            'conditions.*.conditions.*.field_type' => ['required', 'string', 'max:255'],
            'conditions.*.conditions.*.field_value' => ['required'],
            'conditions.*.conditions.*.operand' => ['required', 'string', 'max:255'],
        ];
    }
